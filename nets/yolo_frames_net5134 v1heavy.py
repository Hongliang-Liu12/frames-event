...
class Motion_coupling_Neck(nn.Module):
    ...
    def __init__(self, in_channels, num_frame=3, act="silu"):
        ...
        self.conv_cur = nn.Sequential(BaseConv(in_channels, in_channels, 3, 1, act=act), BaseConv(in_channels, in_channels, 3, 1, act=act))
        self.conv_gl = nn.Sequential(BaseConv(in_channels*2, in_channels*2, 3, 1, act=act), BaseConv(in_channels*2, in_channels, 3, 1, act=act))
        self.conv_gl_mix = nn.Sequential(BaseConv(in_channels, in_channels, 3, 1, act=act), BaseConv(in_channels, in_channels, 3, 1, act=act))
        self.conv_cr_mix = nn.Sequential(BaseConv(in_channels*2, in_channels*2, 3, 1, act=act), BaseConv(in_channels*2, in_channels, 3, 1, act=act))
        self.conv_final = nn.Sequential(BaseConv(in_channels*2, in_channels*2, 3, 1, act=act), BaseConv(in_channels*2, in_channels, 3, 1, act=act))
